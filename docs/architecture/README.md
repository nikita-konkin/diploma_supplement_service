...
Статус: **черновик для обсуждения**, 25.09.2026. Технологический стек выбран — Java 11 LTS, [ADR-0007](adr/0007-technology-stack.md) (раздел 13).
...
1. **Сейчас:** текущий сервис продолжает работать. P0, P1 и P2 из [BACKLOG.md](../BACKLOG.md) исправлены, кроме B-31 (дисциплины на несколько семестров, ждёт ответа владельца); в КиберДипломе заполняется «Местонахождение» (B-26).
...
Рефакторинг текущего сервиса (оценка 25.09.2026):

- **сделано в P1:** в шлюзе один разборщик формы и один HTTP-клиент вместо двух копий; удалены `/config`, `/validate`, «ремонт» файлов, мёртвый дубликат подсветки и отладочная печать; фамилии студентов больше не пишутся в журналы;
- **генератор XML** после P0 в хорошем состоянии (проверки собраны в один отчёт, 36 тестов) — переносится в новую систему по тестам;
- **ядро парсера** держит данные в строке индекса «Название_тип_з.е.», сопоставляет подстроками и меняет индекс таблицы на ходу. Отсюда B-31. Его нужно не рефакторить, а переписать один раз — сразу как модуль импорта новой системы: явная модель строки ведомости (дисциплина, семестр, часы, форма контроля, оценка) → сложение семестров → сопоставление с планом с отчётом. В текущем сервисе — только точечное исправление B-31;
- **сборка:** Docker-сборка шлюза пропускает тесты (`-DskipTests`) — включить; CRLF и прочее — P3 в backlog;
- **три сервиса вместо одного** для текущей задачи избыточны, но объединять их незачем: новая система заменит всю связку.

## 13. Выбор стека

Решение — [ADR-0007](adr/0007-technology-stack.md), принято владельцем 25.09.2026: Java 11 LTS с рантаймом в поставке (Liberica/Axiom JDK); запасной вариант — Go на форке для Windows 7, если прототип не подтвердит платформенные требования. Требования, по которым сравнивались кандидаты:
...
| [0007](adr/0007-technology-stack.md) | Технологический стек: Java 11 LTS с рантаймом в поставке, запасной — Go | принято |
